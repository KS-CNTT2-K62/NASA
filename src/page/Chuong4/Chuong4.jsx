...
function App() {
  ...
  return (
    <div className="bg-gray-100 font-sans">
      <header style={{ backgroundColor: '#1d80c3' }} className="text-white text-center py-20 px-4 relative overflow-hidden">
      ...
      <main>
        ...
        <section style={{ backgroundColor: '#6d28d9' }} className="py-20 px-6 md:px-12 text-white">
          <div className="max-w-6xl mx-auto">
            <h2 className="text-4xl font-bold text-center mb-12">Trạm Thực Hành Dữ Liệu</h2>
            <div className="grid md:grid-cols-1 lg:grid-cols-1 gap-8">
              <PracticeStation title="Hoạt động 1: Xây Dựng Bản Đồ Phenology Cá Nhân" challenge="Alice, cháu hãy 'thu thập dữ liệu' về thời điểm hoa nở của một loài cây cụ thể trong vườn hoặc công viên gần nhà cháu trong 4 tuần. Ghi lại 'ngày nở', 'nhiệt độ trung bình', 'lượng mưa'. Đây là dữ liệu phenology sơ khai của cháu. Sau đó, trực quan hóa nó bằng biểu đồ đường theo thời gian." concepts="Phenology (Sinh thái học thời gian), Quan sát cộng đồng (Crowd-sourced Observations), Dữ liệu phân tán (Scatter Plot).">
                <div className="grid md:grid-cols-2 gap-6 items-center">
                  <div>
                    <h4 className="font-bold mb-2">Nhập dữ liệu của bạn:</h4>
                    <form onSubmit={handleAddData} className="space-y-3">
                      <input type="number" min="1" max="4" placeholder="Tuần (1-4)" value={newDataPoint.week} onChange={e => setNewDataPoint({...newDataPoint, week: e.target.value})} className="w-full p-2 rounded bg-white/20 placeholder-white/70 text-white" />
                      <input type="number" placeholder="Nhiệt độ trung bình (°C)" value={newDataPoint.temp} onChange={e => setNewDataPoint({...newDataPoint, temp: e.target.value})} className="w-full p-2 rounded bg-white/20 placeholder-white/70 text-white" />
                      <input type="number" placeholder="Lượng mưa (mm)" value={newDataPoint.rain} onChange={e => setNewDataPoint({...newDataPoint, rain: e.target.value})} className="w-full p-2 rounded bg-white/20 placeholder-white/70 text-white" />
                      <button type="submit" className="w-full bg-yellow-400 text-purple-900 font-bold py-2 px-4 rounded hover:bg-yellow-300 transition-colors">Thêm vào biểu đồ</button>
                    </form>
                  </div>
                  <div className="w-full overflow-x-auto"><CustomLineChart data={phenologyData} /></div>
                </div>
              </PracticeStation>
              <PracticeStation title="Hoạt động 2: Phân Tích Xu Hướng Nở Hoa Toàn Cầu" challenge="Trên màn hình quả địa cầu, Người Kiến Tạo Tương Lai chiếu dữ liệu lịch sử về thời điểm hoa anh đào nở ở Kyoto (Nhật Bản) trong 100 năm qua. 'Alice, cháu hãy 'phân tích biểu đồ xu hướng' này. Cháu có nhận thấy 'hoa anh đào nở sớm hơn' không? Đây là một phân tích xu hướng (Trend Analysis) quan trọng về biến đổi khí hậu.'" concepts="Dữ liệu lịch sử (Historical Data), Xu hướng dữ liệu (Data Trend), Biến đổi khí hậu (Climate Change Impact).">
                <div className="w-full overflow-x-auto m-auto"><CustomLineChart data={kyotoData} /></div>
                <p className="mt-4 text-center font-semibold">Hãy quan sát đường biểu đồ đi xuống, điều này cho thấy hoa đang nở vào những ngày sớm hơn trong năm theo thời gian.</p>
              </PracticeStation>
              <PracticeStation title="Hoạt động 3: Sáng Tạo Công Nghệ Tương Lai" challenge="Kiến Trúc Sư Tương Lai đưa Alice một 'bảng phác thảo ý tưởng' ảo. 'Alice, nếu cháu có một 'cỗ máy drone' để nghiên cứu hoa, cháu sẽ trang bị cho nó những 'cảm biến' gì (ví dụ: máy ảnh RGB, cảm biến quang phổ, cảm biến nhiệt độ) để thu thập dữ liệu đa quang phổ và dữ liệu môi trường? Hãy 'vẽ concept' và 'giải thích cách hoạt động' của nó.'" concepts="Dữ liệu đa quang phổ (Multispectral Data), Cảm biến (Sensors), Hệ thống tự động (Automated Systems).">
                 <div className="grid md:grid-cols-2 gap-6 items-center">
                    <div>
